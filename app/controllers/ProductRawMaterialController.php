...
class ProductRawMaterialController extends MainController {

    public function index() {
        $raw = new ProductRawMaterialModel();
        $data = $raw->getAll();
        ...
        if ($this->f3->VERB == 'POST' && str_contains($this->f3->get('HEADERS[Content-Type]'), 'json')) {
            ...
            if (strlen($this->f3->get('BODY'))) {
                ...
                if (json_last_error() == JSON_ERROR_NONE) {
                    $raw = new ProductRawMaterialModel();
                    $status = $raw->createMaterial($data);
                    header('Content-Type: application/json');
    ...
    public function read($f3, $params) {
        $raw = new ProductRawMaterialModel();
        $data = $raw->getMaterial($params['id']);
        header('Content-Type: application/json');
        echo json_encode($data);
    }
    ...
    public function update($f3, $params) {
        if ($this->f3->VERB == 'PUT' && str_contains($this->f3->get('HEADERS[Content-Type]'), 'json')) {
            ...
            if (strlen($this->f3->get('BODY'))) {
                ...
                if (json_last_error() == JSON_ERROR_NONE) {
                    $raw = new ProductRawMaterialModel();
                    $status = $raw->updateMaterial($params['id'], $data);
                    header('Content-Type: application/json');
                    echo json_encode($status);
                }
            }
    ...
    public function delete($f3, $params) {
        $raw = new ProductRawMaterialModel();
        $status = $raw->deleteMaterial($params['id']);
        header('Content-Type: application/json');
        echo json_encode($status);
    }
